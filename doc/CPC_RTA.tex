...
\subsubsection{Equations of motion for single particle electronic wavefunctions : TDLDA}
...
\subsubsection{Relaxation-time approximation (RTA)}
\label{sec:RTA}

\paragraph{The formal background of RTA}
...
\paragraph{Photo-electron angular distribution (PAD)}
\label{sec:pad}
...
\paragraph{Photo-emission spectra (PES)}
\label{sec:pes}
...
\subsection{Electronic properties at the level of DFT}

\subsubsection{Ground state}
\EScomm{
statics, convergence issues, energy, sp energies…   1 figure composed with panels
}

\subsubsection{Propagation}

\EScomm{validation of mean field propagation; might be a place to readvertise  the dynamics in natural basis 
and comparaison to standard propagation. Still to be decided in
detail.
It is difficult to show something really telling. 
We should nevertheless mention the propagators tested and, after all, 
this might also be done in relation to natural orbitals. Mention also the importance of time step.
}

\PGR{
PGR2ES: My comments and proposals are:
\\
1. We compare here exponential evolution versus TV-step addressing:
\\
\hspace*{1em}a) precision \& stability (dependence on time step),
\\
\hspace*{1em}b) remaining (small) incompatibility of TV-step with
static g.s.,
\\
\hspace*{1em}c) compare time consumption.
\\
2. Effect of absorbing b.c.\PGRfoot{PGR2ES: I propose to  discuss absorption
  within this section because a separate section would give them too
  muuch weight.} and interpretation in terms of natural
orbitals\PGRfoot{PGR2ES: Natural orbitals in DFT are preferably a means of
  interpretation, not for
  propagation. We discuss as pedagogical introduction
  because they become a basic building block in RTA.}:
\\
\hspace*{1em}a) Discuss (no figures) loss of ortho-normality and why
one still can use the propagating orbitals (concept of full space).
\\
\hspace*{1em}b) Go to natural orbitals. Discuss occupation numbers
(figure $n_i(\varepsilon)$ at finite time). Compare entropy from
nat.orb. with equilibrium entropy for given excitation energy
(as preparation for RTA).
}

\subsubsection{DCMF}
\EScomm{DCMF example; 1 figure on convergence + fulfilling of density/current constraint
}
...
\EScomm{
One multi panel figure for C3, C60, Na21+ and H2O with sp energies 
and optical responses Cf Fig 1 Wopperer Phys Rep
}

\subsection{RTA dynamics}

\subsubsection{Electron dynamics}

\EScomm{Typical laser irradiation example with standard observables on and  off resonance for TDLDA and RTA. 
We make a single figure with 8 panels for : dipole, Nesc,  PES (only TDLDA), and PAD; left panels off resonance, 
right panels on resonance; both TDLDA and RTA on each panel.}

\subsubsection{Ionic dynamics}
\EScomm{One example of coupled ion+electron dynamics following a boost. Show electronic dipole and ionic 
