...
\externaldocument[man:]{User_manual}
...
\author{F.~Coppens$^{a,b,c}$}
\author{P. M. Dinh$^{a,b,*}$}
\author{P.-G.~Reinhard$^c$}
\author{E.~Suraud$^{a,b,d}$}
\author{M.~Vincendon$^{a,b}$}
\cortext[author]{Corresponding author:
  dinh@irsamc.ups-tlse.fr} 
\address{$^a$Universit\'e de Toulouse; UPS; Laboratoire de Physique Th\'{e}orique, IRSAMC; F-31062 Toulouse Cedex, France}
\address{$^b$CNRS; UMR5152; F-31062 Toulouse Cedex, France}
\address{$^c$IRT Saint Exup\'ery, Institut de Recherche Technologique}
\address{$^d$Institut f{\"u}r Theoretische Physik, Universit{\"a}t
  Erlangen, Staudstrasse 7, D-91058 Erlangen, Germany}
\address{$^e$School of Mathematics and Physics, Queen's University Belfast, Belfast, UK}
...
as delivered by lasers or highly charged ions. Basis of the description is
...
approximate Self-Interaction Correction (SIC), the latter being crucial for
...
the rich empirical material from heavy-ion collisions. This was done
...
Early stages of irradiation dynamics proceed at short time scale,
...
Initially (coherent) quantum correlations dominate at very short times. But
...
which also gives access to  larger systems then unfolding to macroscopic scale.
...
and dissipative effects for applications in finite electronic systems
...
\begin{figure}[htbp!]
\begin{center}
 \includegraphics[width=0.78\columnwidth]{figures/qdd_examples}
...
\caption{Ground state properties of 4 typical systems attainable with
  ...
  energies (with the HOMO emphasized in blue and the LUMO in red dashes) and
  the optical response in some specific spatial directions as indicated. }
\label{fig:systems}
\end{center}
\end{figure}
...
\cite{Wop14aR}. We show for all of them the ionic structure dressed by
...
that should be taken as a user manual. At several places, cross-references
to the user manual can be found; they are indicated by a heading
letter ``M'' to avoid any confusion with a section or an equation of the present paper.
...
equilibrium.  As with DFT for the ground state, Time-Dependent
  DFT (TDDFT), mostly treated at the level of the Time-Dependent
  Local-Density Approximation (TDLDA),  approximately accounts for some
...
is still under development.  Fully coherent
...
hope to extend them directly to large systems and/or long times, because of their too demanding 
computational scaling properties.  The
...
systems $\sim$ 10 electrons, short times $\sim$ 10 fs).  On the
...
excitations and the regime of high excitation density  by
...
throughout the process \cite{Din18}. These theories can deal with
large systems ($>$ 100 active electrons) on intermediate to long times
($>$ 100 fs).  They are based on Stochastic TDHF (STDHF) \cite{Sur14}
and different levels of approximation to it.  STDHF describes DC in
terms of a stochastic treatment of electron-electron collisions
leading to an incoherent ensemble of time-dependent mean-field
states. The quantum collision term is evaluated in Markov
approximation, thus neglecting memory effects. The latter depend on
the spectral density and usually decrease with excitation energy
\cite{Gre94}. STDHF has been validated (at high enough excitation
...
Whatever approach, dissipative dynamics thus neglects coherent DC.  Coherence may play a role at the very first
  stages of building up DC, the longer the lower the excitation
  energy. STDHF, ASTDHF, and particularly RTA, are thus best suited to sufficiently
  ...
  processes stays outside the scope of these approaches.

In the present paper, we present our QDD code, an open source implementation of
the RTA on top of a 3D coordinate-space code for TDLDA augmented by a
various versions of Self-Interaction Correction (SIC) \cite{Leg02}.  TDLDA is
...
in a standard manner, as usually done in real-time implementations of TDDFT
...
numbers at the side of {\rm valence} electrons, and classical
...
\end{equation}
Note that $\alpha$ 
also accounts for spin (up, $\uparrow$ and down, $\downarrow$) degrees of freedom.
$\Omega$ denotes the total number of s.p. wave functions. 
It must be $\Omega\geq N_{\rm el}$ for a system with $N_{\rm el}$ valence
...
is the electronic local density which covers, in fact, two separate densities for spins up and
spins down
\begin{subequations}
\begin{equation}
...
\label{eq:locdens}
\end{equation}
which sum up to the total density
\begin{equation}
\varrho(\mathbf{r},t) = \varrho_\uparrow(\mathbf{r},t) + \varrho_\downarrow(\mathbf{r},t) = \sum_{\alpha}w_\alpha \varphi_\alpha^*(\mathbf{r},t)
  \varphi_\alpha(\mathbf{r},t).
\label{eq:totrho}
\end{equation}
\end{subequations}
%To simplify the presentation of the formalism, we
...
\begin{eqnarray}
  ...
  E_\mathrm{kin}
  +
  \underbrace{E_\mathrm{C}
  +
  E_\mathrm{xc}}_{E_\mathrm{LDA}}
  +
  E_\mathrm{ext}
  +
  E_\mathrm{el,ion}
...
\\
  ...
  \varphi_\alpha^+\frac{\hat{p}^2}{2m_e}\varphi_\alpha^{\mbox{}}
  \quad,
\label{eq:Ekin}
\\
  E_\mathrm{C}[\varrho]
...
\\
  E_\mathrm{xc}[\varrho_\uparrow, \varrho_\downarrow]
  &=&
  \int \textrm d\mathbf{r}\,\varrho_\uparrow(\mathbf{r},t)
  \epsilon_\mathrm{xc}\left(\varrho_\uparrow(\mathbf{r},t)\right)
  +
  \int \textrm d\mathbf{r}\,\varrho_\downarrow(\mathbf{r},t)
  \epsilon_\mathrm{xc}\left(\varrho_\downarrow(\mathbf{r},t)\right)
  \quad,
\\
...
\end{eqnarray}
\end{subequations}
where functionals of density are  indicated by square
...
exchange and correlations computed from the exchange correlation functional $\epsilon_\mathrm{xc}$.
The sum of $E_{\rm C}$ and $E_{\rm xc}$ constitute the LDA energy $E_{\rm LDA}$.
We use in the code two options:
...
The LDA exchange-correlation term then becomes a function of the density:
...
energy describes the action of an external field on the ions.
...
of the HOMO. This is a crucial effect in a real-time 
...
applied since the 2000s in clusters~\cite{Leg02}, provides a
...
\label{eq:ADSIC}
\end{equation}
where $N_\sigma$ is the number of electrons with spin $\sigma$.  
ADSIC delivers again a functional of the local density only and thus can be
...
physical situations we are interested in.   There is also another
  case for which  ADSIC is not suited, namely if the system combines
...
There exist more elaborate approaches to full SIC. 
%Some of them are incorporate in the QDD code, however with reduced functionality.  
One option is to treat Coulomb exchange exactly, but then ignoring
  correlations.  The other options belong to the class of optimized
  effective potentials (OEP) which aim at best possible local
  potentials simulating exchange-correlation effects
  \cite{Tal76,Nor84,Kue08}.  The next simple after ADSIC is the
  SIC-Slater approximation \cite{Sla51,Sha53} which accounts for the
  local dominance of particular s.p. states. The method of
  Krieger-Li-Ifrate (KLI) puts a feedback loop on top of SIC-Slater to
  cure deficiencies of the mere SIC-Slater approach \cite{Kri92a}.
  These three methods, SIC-Slater, KLI, and exact exchange, are also
  implemented in QDD, however with reduced functionality: they  do not
  work for fractional occupation numbers. A safe
  application of SIC-Slater and KLI is confined to static calculation
  because time-dependent simulations can easily run into
  insurmountable difficulties \cite{Mun05,Mun07a}.  Nonetheless, we
  find it useful to have these options available for occasional
  checks. For the formal details, see the brief summary in \cite{Leg02}
  or the extensive review on OEP \cite{Kue08}.
...
represented as a sum of Gaussians.  The corresponding potential is then
...
\begin{eqnarray}
  ...
  &=&
  Ze^2\sum_{i=1}^2 c_i 
  \frac{\mbox{erf}(|\mathbf{r}|/(\sqrt{2}\sigma_i))}{|\mathbf{r}|}
  \quad,
\label{eq:locPsP}
\\
  \mbox{erf}(x) 
...
More involved, but also more versatile in the applicability, are the
Goedecker-like pseudopotentials from \cite{Goe96} which are composed
...
\label{eq:Goeproj}
\end{equation} 
where $\Gamma$ is the gamma function and $Y_{lm}$ is the spherical
harmonics, evaluated at the solid angle $\Omega_\mathbf{r}$ about the
position vector $\mathbf{r}$. For the chemical elements we deal with, we use
...
is a standard approach in the theory of bulk metals \cite{Ash76} and the
adaptation to a finite cluster is straightforward. One carves from
bulk jellium a finite element of constant positive charge density
...
$\varrho_{\mathrm{jel},0}={3}/({4\pi r_s^3})$ and the Wigner-Seitz radius $r_s$ is a
genuine material parameter \cite{Ash76}. $\sigma_\mathrm{jel}$
parametrizes the surface width and the transition from 90\% to 10\%
bulk density is achieved within $4\sigma_\mathrm{jel}$. 

The model also allows one to describe deformed clusters by angular
dependence $R(\theta,\phi)$ of the extension. This is achieved through
the deformation coefficients $\alpha_{lm}$ weighting the impact of the
spherical harmonics. Actually, we use in the code only the quadrupole
deformations $\alpha_{20}$ and $\alpha_{2\pm2}$ as well as the
hexadecapole deformation $\alpha_{40}$.  Axially symmetric shapes are
obtained if $\alpha_{2\pm2}=0$ where then positive $\alpha_{20}$
produce prolate shapes and negative $\alpha_{20}$ oblate ones. The
hexadecapole moment plays a role for fine-tuning the shape
\cite{Mon95b}.
%
The cluster radius
$R_\mathrm{jel}$ is fixed by the other parameters through
Eq. (\ref{eq:R0cond}) which is to be solved numerically by a root
finding procedure, see the Supplemental material, section \ref{man:sec:jelliter}.

After all, the leading parameters of the soft
jellium model (\ref{eq:softJ}) are the Wigner-Seitz radius $r_s$
and the surface thickness $\sigma_\mathrm{jel}$.  They are universal
for a given material. Typical values are $r_s\sim 4\,\mathrm{a}_0$
with $\sigma_\mathrm{jel}\sim 0.9\,\mathrm{a}_0$ for Na clusters,
$r_s\sim 2.66\,\mathrm{a}_0$ and $\sigma_\mathrm{jel}\sim
0.76\,\mathrm{a}_0$ for {Mg clusters}, or $r_s\sim 3\,\mathrm{a}_0$
and $\sigma_\mathrm{jel}\sim 0.78\,\mathrm{a}_0$ for Ag clusters.  The
deformation parameters $\alpha_{lm}$ depend on the actual cluster and
strongly vary with system and size.
...
situations where ionic motion can be ignored. Note that QDD offers the
possibility to consider an even simpler modeling option by means of
phenomenological electronic shell models of harmonic-oscillator or
Woods-Saxon type \cite{Bra93,Rei04aB}. This can be done in very
flexible manner by supplying the shell model choice in form of an
explicit 3D field and reading it as background potential, for details,
see the Supplemental material, section~\ref{man:sec:perio}.
...
subsection, together with two further instantaneous
excitation schemes for more subtle modes (spin dipole, scissors).
...
can also neglect the effect of the magnetic field for the laser 
intensities of relevance here.  The coupling
...
\label{eq:laserp}
\end{equation}
where $f(t')$ is the envelop of the laser pulse.
...
$T_\mathrm{pulse}$ is identical with the FWHM of a Gaussian pulse.
...
cluster physics.  Fortunately enough there exists a class of cases
which can be addressed in a simplified, more robust way: highly
charged ions, protons and, to some extent, also electrons can be
considered as being structureless. What only counts is their Coulomb
field.  Charged ions are heavy and can be treated with classical
trajectories
%
...
the system can be described by a time-dependent external field \cite{Wan11}:
\begin{subequations}
\label{eq:ionext}
\begin{eqnarray}
...
\\
  \mathbf{R}_\mathrm{ext}(t)
  &=&
  \mathbf{R}_\mathrm{ext}(0)+\dot{\mathbf{R}}_\mathrm{ext}(0) t
  \quad.
\end{eqnarray}
\end{subequations}
...
$v_\mathrm{ion}$ and the impact parameter $b_\mathrm{ion}$ \cite{Can13}.
The velocity is more or less well
...
examples see \cite{Rei98a,Bae06a}.

More involved projectiles come into play when considering atomic
collisions. Such scenarios could, in principle, also be treated in QDD
and it had been investigated in the past in connection with collisions
on surfaces \cite{Din09aR}. However, the preparation of the initial
condition is much handwork because it has too many degrees of freedom
and has to be done in several steps. We thus do not discuss this
option further in this public version of the QDD code.

\paragraph{Instantaneous boost}
\label{sec:boost}

If the time of impact from a laser pulse or a charged projectile is
shorter than the typical response time of  occupied electron states, the
pulse just  instantaneously imprints a certain phase profile onto the
...
$E_\mathrm{boost}=\displaystyle \frac{N_\mathrm{el}}{2m_e} |\mathbf p_0|^2$. Such an excitation
...
frequencies.  As  a consequence, the response of the system is
...
section \ref{sec:specan}. The code also provides  the option of an
instantaneous displacement of the electron cloud which is a
zero-time excitation as the boost, but with bias to higher
energies.

\subsubsection{Further instantaneous excitations}
\label{sec:iniexo}

There are two more instantaneous excitations schemes built into the QDD code
which trigger less prominent modes, namely spin-dipole and scissors
oscillations. The spin-dipole mode is initiated by a spin-dipole boost
(Eq. (\ref{man:eq:spindipboost}) in the Supplemental material)
similar as the dipole boost
in Eq.~(\ref{eq:boost}), however spin-up and spin-down boosted in
opposite directions. The mode is tracked by spin-dipole moment,
see section \ref{sec:obsexo}.

The scissors mode is an orbital $J=1^-$ excitation representing a
rotational oscillation of one principle axis of the electron cloud
against the corresponding ionic axis \cite{Lip89,Nes99}.  It is
initialized by an instantaneous rotation (\ref{man:eq:rotboost}) of the ionic background
relative to the electron cloud. This mode is tracked by the angular
momentum of the electron cloud, see  section \ref{sec:obsexo}.

\subsection{Equations of motion}
\label{sec:mf}

The degrees of freedom of the theory are the s.p. wave functions
$\varphi_\alpha$, their occupation number $\omega_\alpha$, and ionic
positions and momenta ${\bf R}_I,{\bf P}_I$. We derive in this section
equations of motion for each of them.  Those for the
$\varphi_\alpha$'s as well as for the ${\bf R}_I$'s and ${\bf P}_I$'s
are obtained from a variational principle on the total energy
$E_\mathrm{total}$. For fixed $\omega_\alpha$, the resulting equations
of motion finally deliver coupled TDLDA-MD equations in which
electrons are propagated according to TDLDA equations and ions
according to classical Molecular Dynamics (MD). Such coupled dynamics
is often quoted Ehrenfest dynamics \cite{Tod01a}.  The dynamical
equations for the $\omega_\alpha$'s requires a dedicated sequence of
approximations well documented in the literature \cite{Bal75}.  We
shall not enter those details here and just recall briefly the path
towards the RTA we use to effectively
propagate the $\omega_\alpha$'s.
...
\label{eq:KS}
\begin{eqnarray}
  \hat{h}_{\sigma_\alpha}\varphi_\alpha
  &=&
...
\\
  ...
  \underbrace{
    ...
    +
    \hat{V}_{\mathrm{xc}}[\rho_\sigma]
    +
    \hat{V}_\mathrm{el,ion}
    +
    V_\mathrm{ext} (\mathbf{r},t)
  }_{V_\mathrm{KS}}
  \quad,
\\
  ...
  \quad,
\\
  \hat{V}_{\mathrm{xc},\sigma} %(\mathbf{r})
  &=&
  \epsilon_\mathrm{xc}\left[\varrho_\sigma(\mathbf{r},t)\right]
  +
  \varrho_\sigma(\mathbf{r},t)
  \left.\frac{\delta\epsilon_\mathrm{xc}[\rho]}
             {\delta\rho}\right|_{\rho=\varrho_\sigma(\mathbf r,t)}
  \quad,
\label{eq:Uxc}
\end{eqnarray}
where $\hat{h}$ is the KS mean-field Hamiltonian and
$\sigma\in\{\uparrow,\downarrow\}$.  The exchange-correlation
potential $\hat{V}_{\mathrm{xc}}$ involves the construction
$\delta\epsilon_\mathrm{xc}[\rho]/{\delta\rho}$ which is the
functional derivative of the exchange-correlation energy per particle
$\epsilon_\mathrm{xc}$ with respect to the local density $\rho$
considered as an independent variable. This functional derivative is
then evaluated at $\rho=\varrho_\sigma(\mathbf r,t)$.
$\hat{V}_\mathrm{el,ion}$ is the potential describing the interaction
of the electrons with the ions, and is either a pseudopotential
$\hat{V}_\mathrm{PsP}$, see section \ref{sec:practPsP}, or stems from
the jellium model, see section \ref{sec:jell}.
...
\end{equation}
\end{subequations}
where $\hat{h}$ is composed in the same manner as in the
...
So far, we dealt with the equations for pure states.  In the case of
stationary states at finite temperature and in the case of dissipative dynamics, we
encounter mixed states.  This is described by associating an
occupation number $w_\alpha$ with each s.p. state
...
\begin{equation}
  ...
  \sum_{\alpha=1}^\infty|\varphi_\alpha\rangle w_\alpha\langle\varphi_\alpha|
   \simeq 
   \sum_{\alpha=1}^\Omega|\varphi_\alpha\rangle w_\alpha\langle\varphi_\alpha|
 \quad.
\label{eq:rhodiag}
\end{equation}
...
As said above, the occupation numbers $w_\alpha$ are kept frozen
...
\label{eq:KSrho}
\end{equation}
where $\hat{h}[\varrho_\uparrow,\varrho_\downarrow]$ is the KS
Hamiltonian as above.  This pure mean-field propagation
(\ref{eq:KSrho}) leaves the occupation numbers $w_\alpha$'s unchanged
and propagates only the s.p. states.  The mean-field propagation of an
initial state (\ref{eq:rhodiag}) then reads:
\begin{eqnarray}
  \hat{\rho}(t)
  &=&
  \sum_{\alpha=1}^\infty
  |\varphi_\alpha(t)\rangle w_\alpha\langle\varphi_\alpha(t)|
  \simeq \sum_{\alpha=1}^\Omega|\varphi_\alpha\rangle w_\alpha\langle\varphi_\alpha|
%\nonumber\\  
%  &=&
  =
...
To simplify the rather involved notations, we drop the spin index
  throughout this RTA part.
  
The quantum Boltzmann equation is the quantum mechanical counterpart
of the semiclassical Vlasov-Uehling-Uhlenbeck equation
\cite{Ber88,Abe96}.  It complements the self-consistent TDLDA
propagation of the one-body density operator $\hat{\rho}$ by dynamical
correlations through a collision term. It reads in general
\cite{Rei85f,Goe86a}
%\begin{eqnarray}
...
%\\  
  ...
  \;,\;
  r_s^\mathrm{(el)}=\left(\frac{3\bar{\varrho}}{4\pi}\right)^{-1/3}
  \;,\;
...
\begin{equation}
  ...
  E_\mathrm{sp}
  =
  \sum_\alpha w_\alpha\varepsilon_\alpha =  \sum_\alpha w_\alpha \langle \varphi_\alpha | {\hat h}| \varphi_\alpha \rangle
  \quad.
\end{equation}
...
in other words, changes in total energy $E$ at frozen $\varrho$ are
identical to changes in s.p. energy $E_\mathrm{sp}$. Thus we will use
$E_\mathrm{sp}$ in many of the subsequent equations whenever
evaluating changes of $E$, but not always write it explicitely.
...
intrinsic (thermal) energy of the system $E^*_\mathrm{intr}$ (defined
later on in Eq. (\ref{eq:Estar}), the actual number of valence electrons $N_{\rm
  el}$, the in-medium electron-electron cross-section $\sigma_{ee}$,
the effective Wigner-Seitz radius $r_s^\mathrm{(el)}$ of the electron cloud, and the
average electron density $\bar{\varrho}$.  Note that $r_s^\mathrm{(el)}$
 and
$\sigma_{ee}$ depend on the average density $\bar{\varrho}$ because a
spatially varying $\tau_\mathrm{relax}$ would be very cumbersome to
implement in a quantum mechanical expression.  The average density is
deduced from the r.m.s. radius $r_\mathrm{rms}$ of the actual electron
cloud as
$\bar{\varrho}=3N_\mathrm{el}/\left[4\pi(\sqrt{5/3}\, r_\mathrm{rms})^3\right]$.
...
at the given instant. The $\hat{\varrho}(\mathbf r)$ is the
  operator of local density at the space point $\mathbf{r}$ and
  $\hat{\mathbf{j}}(\mathbf r)$ the operator of local current.  The
wanted energy $E$ (or $E_\mathrm{sp}(t)$ equivalently) and electron
number $N_\mathrm{el}$ are tuned by adjusting chemical potential
$\mu^\mathrm{(eq)}$ and temperature $T^\mathrm{(eq)}$ in the
...
\\[3pt]
  &=&
  \sum_\alpha|\varphi^\mathrm{(eq)}_\alpha\rangle  w^\mathrm{(eq)}_\alpha
     \langle\varphi^\mathrm{(eq)}_\alpha|
\end{array}$
\\[4pt]
...
relaxation time:
\\[4pt]
$\hbar\tau_\mathrm{relax}^{-1}=0.40\,\sigma_{ee}(r_s^\mathrm{(el)})^{-2}\,E^*_\mathrm{intr}/N_{\rm el}$
\end{flushleft}
\end{minipage}
}}
...
\begin{flushleft}
$\Delta^2\hat{h}_\mathrm{DCMF}\stackrel{\mbox{?}}{<}\epsilon_1$
...
$\sqrt{\Delta^2\hat{h}_\mathrm{DCMF}}\stackrel{\mbox{?}}{<}\epsilon_0$
$\;\&\;$
$|E_\mathrm{sp}^\mathrm{(new)}-E_\mathrm{sp}^{(n)}|\stackrel{\mbox{?}}{<}\epsilon_2$
\\
$\sum_\mathbf{\nu}|\rho_\mathrm{DCMF}(\mathbf{r}_\mathbf{\nu})-\rho(\mathbf{r}_\mathbf{\nu},t)|/\sum_\mathbf{\nu}
...
\lambda^\mathrm{(n+1)}
\\
\boldsymbol\lambda_j^{(n)}
+
2\mu_j(\mathbf{j}^\mathrm{(new)}-\mathbf{j})%_\mathrm{mf})
\!\!&=&\!\!
\boldsymbol\lambda_j^\mathrm{(n+1)}
\end{array}
$
\end{flushleft}
\end{minipage}
...
operator in the accelerated gradient step, see
Eqs.~(\ref{eq:dampstep}),
however, with different damping parameters
$E_{0,\mathrm{damp}}^{\mbox{}}\rightarrow
E_{0,\mathrm{damp}}^\mathrm{(RTA)}$
and  
$\delta\rightarrow\delta^\mathrm{(RTA)}$.
 }
\end{figure}


We briefly summarize the solution
scheme for one step from $t\equiv t_0$ to $t_0\!+\!\Delta t\equiv t_1$,
...
$\delta t$ which is much smaller (by a factor 50--200) than the RTA step
...
\begin{enumerate}
   \item\label{it:TDLDA} We first propagate $\hat{\rho}$ by pure
     ...
     |\varphi_\alpha^\mathrm{(mf)} (t_1)\rangle=\hat{U}(t_1,t_0)|\varphi_\alpha(t_0)\rangle$,
     while the occupation numbers $w_\alpha(t_1)=w_\alpha(t_0)$ are
   ...
   \item\label{it:natorb} Absorbing bounds (see section~\ref{sec:abso})
    may have removed parts from
     the s.p. wave functions and so destroy orthonormalization.  We
     ...
     together with corresponding occupation numbers
     $\{\varphi_\alpha^\mathrm{(nat)},w_\alpha^\mathrm{(nat)}\}$, for
     details on the procedure see appendix \ref{sec:natorb}.  This
   ...
   \item\label{it:DCMF} We determine the thermal mean-field
     ...
     $\{|\varphi^\mathrm{(eq)}_{\alpha}\rangle\}$ and new occupation numbers
     $w^\mathrm{(eq)}_\alpha$ in diagonal form (\ref{eq:rhodiag}).
     ...
     \label{eq:Estar}
     \end{equation}
     where $w_\alpha^{(T=0)}$ are the occupation numbers from a
       Fermi distribution (\ref{eq:Fermi}) at temperature zero, i.e. a
       step function. The chemical potential $\mu^\mathrm{(T=0)}$ is
       tuned to match $\sum_\alpha w_\alpha^\mathrm{(T=0)}=N_{\rm el}$
       using the given s.p. energies
       $\langle\varphi_\alpha^\mathrm{(eq)}|\hat{h}|\varphi_\alpha^\mathrm{(eq)}\rangle$.
This $E^*_\mathrm{intr}$ thus measures the
   ...
   \item \label{it:compo} We compose the new density operator as mixture
     ...
     actual intrinsic excitation energy $E^*_\mathrm{intr}$, see Eq.~\eqref{eq:Estar}
...
%   \item \label{it:natural} 
     ...
     preliminary new occupations $\tilde{w}_\alpha$. Technical details
     of this mixing step are found in appendix \ref{sec:mix}.
   ...
   \item \label{it:therm} 
     ...
     occupation numbers $w_\alpha(t_1)$ which comply with
     energy conservation.
\end{enumerate}
...
\ref{fig:summaryDCMF}.  There are three termination criteria in DCMF
iteration: convergence of the variance of s.p. energies $\epsilon_0$,
energy convergence $\epsilon_2$, and convergence of average deviation
of local density $\epsilon_\rho$.  These are given as input
parameters, for details see the Supplemental material.  Note, that the
actual thresholds for the values of these criteria may change with the
system.  As a rough guide, energy criteria scale with Fermi energy and
density with average density of a system. An explicit example of
convergence is discussed in section~\ref{sec:dcmf}.
...
discussed in the Supplemental material, see section
\ref{man:sec:rtaparams} therein.
...
smaller radius in the pseudopotential min$(r_{\rm loc},r_{l})$ in
...
\begin{equation}%\begin{split}
  \big(1-\frac{N_x}{2}\big)\delta x,\;
  \big(2-\frac{N_x}{2}\big)\delta x,\;
  \ldots,\;\big(\frac{N_x}{2}-1)\delta x,\;\frac{N_x}{2}\delta x \quad. 
%\end{split}
\end{equation}
...
transform. This delivers high precision at affordable expense.
...
Transform (FFT). To that end, a forward transform
...
$(\I{k}_n)^m\tilde{f}(k_n)$ to come back to coordinate space.
...
Hamiltonian ${\hat h}$ (\ref{eq:TDKS}) under the constraint of
orthonormality of s.p.  wave functions $\varphi_\alpha$, i.e.
...
\\
  \hat{\mathcal{D}}
  &=&
  \frac{\delta}{\hat{T} + E_{0,\mathrm{damp}}}
\label{eq:dampop}
\end{eqnarray}
\end{subequations}
...
the step size $\delta$ and the damping regulator $E_{0,\mathrm{damp}}$. The latter
should be chosen typically of the order of the depth of the local
potential $V_\mathrm{KS}$ ($E_{0,\mathrm{damp}}
  \simeq|\varepsilon_1|$ where $\varepsilon_1$ is the lowest s.p. energy).  Typical values are
$E_{0,\mathrm{damp}}=10, \ldots,50$ eV,
depending on the material under consideration. After
proper choice of $E_{0,\mathrm{damp}}$, the step size is of order of
$\delta=0.1,\ldots,0.5$. Larger values yield faster iteration but can run
more easily into pathological conditions.

In some cases, an initial finite electronic temperature,
  associated with Fermi distribution (\ref{eq:Fermi0}), can help the
  convergence of the statics, see e.g. section
  \ref{man:sec:open-shell}.  In that case, we have to realize that the
  wave function step has produced a new set of s.p. energies
  $\varepsilon_\alpha^{(n+1)}=\langle\varphi_\alpha^{(n+1)}|\hat{h}^{(n+1)}|\varphi_\alpha^{(n+1)}\rangle$,
  which requires that we determine preliminarily new occupation
  numbers $\tilde{w}_\alpha$ with Eq.  (\ref{eq:Fermi0}).  Using the
  $\tilde{w}_\alpha$ immediately can occasionally lead to unstable
  iteration. It is safer to mix old and new occupation numbers gently
  as
\begin{equation}
  w_\alpha^{(n+1)}
  =
  \eta_\mathrm{occ}\tilde{w}_\alpha
  +
  (1-\eta_\mathrm{occ})w_\alpha^{(n)}
  \quad.
\label{eq:occmix}
\end{equation}
Typical values of mixing lie around $\eta_\mathrm{occ}=1/2$. Critical
cases may be stabilized with smaller values and forgiving cases can
cope with larger values.
...
that we have reached a stable energy minimum, i.e. a solution of the
KS equations. However, this may be only a local minimum (isomeric
...
turns out that $\overline{\Delta\varepsilon}$ provides a stringent
test of convergence, much superior to the mere decrease of total
energy as often used. Alternative convergence criteria, also used in
other codes, can be derived from checking the convergence of the local
...
\end{eqnarray}
\end{subequations}
This yields the density change per grid point. As reference
density, one should take the average density 
$\bar\rho=3N_\mathrm{el}\big/\left[4\pi\displaystyle(\sqrt{5/3}\,r_\mathrm{rms})^3\right]$ with
$r_\mathrm{rms}$ being the r.m.s. radius of the actual electron density. The dimensionless ratios $\delta^{(n)}\rho/\bar\rho$ or
$\delta_2^{(n)}{\rho}/\bar\rho$ allow the comparison of the convergence between
different systems.
%
...
prints the other criteria on demand).
...
energy
$\varepsilon_\alpha^{(0)}=\hbar\omega_xn_x^\alpha+\hbar\omega_yn_y^\alpha+\hbar\omega_zn_z^\alpha$
...
A word of caution is in order about open-shell systems, i.e. systems
where the HOMO-LUMO gap is extremely small or zero.  There can occur
situations which have no DFT solution. The C atom is an emblematic
example in that sense. It has four valence electrons.  Two of them
occupy the $1s$ state in the pseudo atom (frozen core
electrons). There remain two spin up electrons (total spin has to be
one) to be placed into the $1p$ shell with three available spin-up
states.  However, any choice of placing the two electrons leads
  to an unstable KS iteration.  Thus one should check not only the
numerics but also the physics if a system refuses to converge. A
finite initial electronic temperature can help sometimes but not
systematically. An instructive example is given in the Supplemental
material, see section \ref{man:sec:open-shell}.
...
Dynamical cooling allows one to better explore the energy landscape and
...
ionic kinetic energy. Starting from a non-minimal energy configuration, the
...
annealing, for a detailed description see \cite{All87aB}.  Simulated
...
experience to use it efficiently \cite{All87aB}, for details see
section \ref{man:sec:basicDYN} in the Supplemental material.
...
methods starts directly from the formulation of the propagator $\hat{U}$
(\ref{eq:KSpropag}). The first method, called exponential evolution,
 simply employs a Taylor expansion of the propagator:
\begin{eqnarray}
...
$m=4$ for the predictor and $8$ for the corrector.
...
speed of change of $\hat{h}(t)$. Altogether, this
...
$\hat{h}$. The smaller the commutators, the better it is.
...
\caption{Comparison of electronic propagation schemes taking as test
  ...
  panels: time evolution of dipole moments. Upper panels: time
...
(\ref{eq:TVsplit}). This propagation is manifestly unitary (in that case without non 
local pseudo potential) such that
...
$\delta t$ as can be seen in the figure (mind the scales, though). Once we have accepted the
initial error, the $TV$ propagation proceeds in stable manner for
long. The larger $\delta t=4.8$~as produces gradually increasing
...
transform is associated with periodic boundary conditions where
...
\cite{Poh04b} (simpler rectangular masks may be used if PAD and PES are not of
interest).  

The limiting radii need yet to be specified. The outer radius is the
smallest distance from the origin of the box to the bounds, formally
$R_\mathrm{out}=\mbox{min}_{\mathbf{r}_\mathbf{\nu}\in\mathrm{bounds}}(|\mathbf{r}_\mathbf{\nu})|)$
where the index $\mathbf{\nu}$ indicates that
$\mathbf{r}_\mathbf{\nu}$ is a grid point. The inner radius is then
\begin{equation}
  R_\mathrm{in}
  =
  R_\mathrm{out}
  -
  N_\mathcal{M}\delta x
  \quad,
\label{eq:abspoints}
\end{equation}
where $\delta x$ is the grid spacing.
The $N_\mathcal{M}$ characterizes the number of absorbing points in
one direction.  It is a crucial numerical parameters, see table
\ref{man:tab:dyn-input-params-general} in the manual.
...
the s.p. basis remains orthonormal by construction. The change of
electron content caused by the absorbing boundary conditions then
leads to a change of occupation numbers in the natural basis.  Both
...
equation (\ref{eq:hDCMFeq}) for which we use again the accelerated
gradient step (\ref{eq:dampstep}) with its own damping parameters
however, with different damping parameters
$E_{0,\mathrm{damp}}^{\mbox{}}\rightarrow
E_{0,\mathrm{damp}}^\mathrm{(RTA)}$ and
$\delta\rightarrow\delta^\mathrm{(RTA)}$. New is the iteration of the
Lagrangian parameters $\lambda(\mathbf{r})$ and
$\boldsymbol{\lambda}_j(\mathbf{r})$ for density and current
constraint in the DCMF mean-field Hamiltonain (\ref{eq:hDCMF}). These
are driven by the mismatch of density and current as
\begin{equation}
\begin{array}{rcl}
  \lambda^\mathrm{(n+1)}
  &=&
...
should be large enough to drive efficiently. However too large values
slow down iterations again.  A proper compromise for them has to be
found and it depends on the actual system. For more details and an
example, see the supplemental material, section~\ref{man:sec:RTA}.
The constraint iteration adds one more termination criterion.  As in
the accelerated gradient step, we check for convergence of energy in
terms of energy change $\Delta
E_\mathrm{sp}=|E_\mathrm{sp}^\mathrm{(n+1)}-E_\mathrm{sp}^\mathrm{(n)}|$
and of variance of s.p. energies $\sqrt{\Delta^2h_\mathrm{DCMF}}=(1/N_e)(
\sum_\alpha w_\alpha^{(n)}\langle \varphi_\alpha^{(n)} |
(\hat{h}_\mathrm{DCMF})^2|\varphi_\alpha^{(n)}\rangle - \left(
w_\alpha^{(n)}\langle \varphi_\alpha^{(n)} |
\hat{h}_\mathrm{DCMF}|\varphi_\alpha^{(n)}\rangle \right)^2)$, see also
figure~\ref{fig:summaryDCMF} in section~\ref{sec:summaryRTA}.  New is
a check for convergence of density in terms of deviation from the
goal
\begin{subequations}
\begin{eqnarray}
...
convergence of $\delta\rho_\mathrm{DCMF}$.



The DCMF scheme also requires to tune the Fermi distribution to the
...
%\ES{$|E_\mathrm{sp}^\mathrm{(n+1)}-E_\mathrm{sp}^\mathrm{(n)}|$},
...
%\end{center}
%\end{figure}
The most critical and time consuming part in the RTA step is the
solution of the DCMF equations (\ref{eq:DCMF}) to obtain the
local instantaneous equilibrium state.  Figure \ref{fig:dcmf} shows an
example of typical convergence pattern for DCMF during RTA evolution.
\begin{figure}[htbp]
\begin{center}
% \includegraphics[width=0.7\columnwidth]{figures/dcmf_cv}
 \includegraphics[width=\columnwidth]{figures/Na8-RTA-2}
\caption{Illustration of convergence of the RTA step for the test
    case Na$_8$ with jellium background excited by a laser pulse with
    frequency $\omega_\mathrm{las}=2.6$ eV, duration $T_\mathrm{pulse}=25$ fs, and
    intensity $I=5\times10^{11}$ W/cm$^2$.  Left column: as functions of time, 
    $z$-dipole moment for RTA dynamics compared with mere TDLDA (bottom) and 
    number of DCMF iterations in each RTA step (top). 
    Right column: as functions of the number of DCMF iterations, deviation of local density from goal (bottom)
    and average variance of s.p. energies (top), both for three times as
    indicated and highlighted by vertical lines in the upper left panel.}
%}
\label{fig:dcmf}
\end{center}
\end{figure}
The left lower panel shows the dipole response to the external laser
field to demonstrate the dynamical regime in which the example works.
The difference between RTA and TDLDA indicates the times at which
dissipation is most active, namely in the early stages where the two
curves start to deviate and where the impact of the laser amplitude
(e.g. in terms of energy input) is large. 
%
The left upper panel shows the number of DCMF iterations which are
needed until the required precision demands are fulfilled. This
number changes dramatically along the dynamical path and it is, indeed,
largest where dissipation has most work. It is to be noted that the
needed number of iterations sensitively depends on the system and
strength of excitation. In the present case, we could afford to set a
limit at 1000 iterations. There are situations where one needs
considerably more. 
%
The right column of the figure shows detailed convergence patterns  
in the top panel, variance of s.p. energies $\sqrt{\Delta^2h_\mathrm{DCMF}}$
and, in the bottom panel, deviation of density $\delta\rho_\mathrm{DCMF}/\bar\rho$, 
see Eq.~(\ref{eq:devrhoDCMF}), at three different times. The convergence
criteria are not simply going downward, particularly in the slowest
case. The interplay of energy optimization and density matching in
DCMF can lead to significant fluctuations in the iteration process.
%
After all, with diligent tuning of the convergence parameters and a
bit of patience, we obtain the nice result in the left lower panel
which demonstrates that proper dissipation eliminates the unphysical
long-term oscillations of a resonantly excited system.
...
subscript $I$ that labels the ionic core (we correspondingly label the ionic mass $M$). We denote by $\delta t_I$
...
\begin{eqnarray}
  ...
  &=&
  \mathbf{R}(t) + \delta t_I \, \mathbf{P}\left(t\!+\!{\textstyle\frac{1}{2}}\delta t_I\right)/M
  \quad,
\label{eq:Verlet1}\\
...
moment when it is needed, because it was  computed using the
...
for ionization, leaving a charged molecule out of Coulomb equilibrium
and still with a considerable amount of electronic excitation energy as
...
less than 1 eV, while the initial boost brings almost 30~eV in the
...
excitation  energy delivered to the system.
...
$\varrho(\mathbf{r},t)$ as a natural outcome.  The electronic density
together with the ionic configuration $\{\mathbf{R}_I\}$ constitutes
the shape of a system in all detail. But this can become an
overwhelming amount of information. The gross structure of a shape is
more simply sorted in terms of its multipole moments.  Leading
quantity is the root-mean-square (r.m.s.) radius which reads for the
electrons:
\begin{subequations}
\label{eq:moments}
\begin{equation}
  r_\mathrm{rms}(t)
  =
  \sqrt{\frac{\int\mathrm{d}\mathbf{r}\,\varrho(\mathbf{r}+\mathbf{r}_\mathrm{cm},t)\,r^2}
       {N_{\rm el}}}
\end{equation}
where $\mathbf{r}_\mathrm{cm}$ is the electronic center of mass. 
Next come the quadrupole moments which quantify the deformation.
The QDD code computes and prints the Cartesian quadrupole
moments
\begin{equation}
  Q_{ij}
  =
  \int\mathrm{d}\mathbf{r}\,\varrho(\mathbf{r}+\mathbf{r}_\mathrm{cm},t)\,
  \left(r_ir_j
  -\frac{\delta_{ij}}{3}\mathbf{r}^2\right)
  \quad.
\label{eq:quadmom}
\end{equation}
\end{subequations}
This is provided not only for the electronic density but also for the
ionic configuration by replacing $\int\mathrm{d}\mathbf{r}\,\varrho
...$ through $\sum_I ...$.

The quadrupole moments (\ref{eq:quadmom}) as such change scale with
system size. This inhibits simple comparison of shapes between systems
of different size. To that end, it is advisable to work with
dimensionless quadrupole moments which can easily be deduced from the
above radius and quadrupole moments. This is outlined in appendix
\ref{sec:dimlessquad}.
...
$V_\mathrm{ext}(\mathbf{r})=e\mathbf{E}_0^{({\rm stat})}\!\cdot\!{\bf r}$ and
performs static calculations for a couple of $\mathbf{E}_0^{({\rm stat})}$.  This
delivers an electric dipole moment
$e\bar{\bf D}=e\langle \mathbf{r}\rangle=e\int \textrm d\mathbf{r} \, \mathbf{r} \varrho(\mathbf{r},t)$ as
a function of $\mathbf{E}_0^{({\rm stat})}$. The tensor of static dipole
polarizability is then
\begin{equation}
  (\alpha_D)_{ij}
  =
  e\frac{\partial\bar{D}_i}{\partial{E}_{0,j}^{({\rm stat})}}\Big|_{\mathbf{E}_0^{({\rm stat})}=0}
...
of long wavelengths, the laser field at the system site is related to
the dipole moment $\mathbf{D}=\mathbf{r}$ where $\mathbf{r}$ is
taken with respect to the center-of-mass of the electron cloud.  Within
...
$ \varphi_\alpha(\mathbf{r},t\!=\!0^+) \leftarrow
  ...
  \varphi_{\alpha}(\mathbf{r},t\!=\!0)$, 
see section~\ref{sec:boost}.  One then propagates
electrons with TDLDA and samples a protocol of the dipole moment
\begin{subequations}
\begin{equation}
  {\bf D}(t)
  =
  \int \textrm d{\bf r}\,{\bf r}\, \varrho({\bf r},t)
  \quad.
\end{equation}
...
$\Im\{\tilde{D}_i(\omega)\}$ while $n=2$  usually performs satisfyingly
...
appropriate observable is then the power spectrum \ref{eq:powerspec}.


The same principles of spectral analysis can also be applied for
computing the vibration spectra of clusters and molecules. Here we concentrate on
...
states, and the ionic multipole moments ought to be used for the
spectral analysis of ionic motion \cite{Rei02d}.

\subsubsection{Momenta probing exotic modes}
\label{sec:obsexo}

As explained in section \ref{sec:iniexo}, QDD also allows one to
excite more subtle modes as spin-dipole and scissors
oscillations. These modes require, of course, specific observables for
proper analysis. The spin-dipole mode is tracked by the spin-dipole
moment
\begin{equation}
  {\bf D}_S(t)
  =
  \int \textrm d{\bf r}\,{\bf r}\, \varrho_\uparrow({\bf r},t)
  -
  \int \textrm d{\bf r}\,{\bf r}\, \varrho_\downarrow({\bf r},t)
  \quad.
\end{equation}
The observable related to the scissors mode is 
the orbital angular momentum 
\begin{equation}
  \mathbf{L}
  =
  \sum_\alpha w_\alpha
  \langle\varphi_\alpha|\hat{\mathbf{r}}\times\hat{\mathbf{p}}
   |\varphi_\alpha\rangle
\label{eq:angmom}
\end{equation}
where $\mathbf{r}$ is to be taken with respect to the center-of-mass
...
total
ionization, ionization probabilities, photo-electron angular distribution (PAD), and
photo-electron spectra (PES). We discuss them in the next four
paragraphs.
...
escaped electrons denoted by $N_\mathrm{esc}$. 
The, now decreasing, norm of the s.p. wave function $\varphi_\alpha$ defines 
the depletion $N_\alpha$ of state $\alpha$, from which one computes the ionization from this state,
$N_{\mathrm{esc},\alpha}$, as the complement of $N_\alpha$:
\begin{equation}
N_\alpha(t)=\langle \varphi_\alpha(t)|\varphi_\alpha(t)\rangle \quad \mathrm{and} \quad
N_{\mathrm{esc},\alpha}(t)=1-N_\alpha(t) \quad.
\label{eq:depletion}
\end{equation}
The total ionization then reads as the sum of the level ionizations:
\begin{equation}
  N_\mathrm{esc}(t)
  =
  \sum_{\alpha=1}^{\Omega} \omega_\alpha N_{\mathrm{esc},\alpha}(t)
\label{eq:nesc}
\end{equation}
%Eqs.~(\ref{eq:depletion}) and (\ref{eq:nesc}) shows} that we have access to more than the mere total
%ionization. 
Both, total ionization and detailed level
depletion as functions of time, are very instructive observables \cite{Din12c}.

\paragraph{Ionization probabilities}
\label{sec:ionizprobab}

Total ionization can be further analyzed in terms of detailed
ionization probabilities. The probability to find the system at time
$t$ in one of the possible (positive) charge states $k$ to which the system can
ionize is denoted by $P^{(+k)}(t)$. 
%
A scheme to estimate the $P^{(+k)}(t)$ has been derived for
pure mean-field states with integer occupation probabilities by
combinatorial analysis \cite{Ull97a}. We summarize it here briefly.

The physical change of charge state is related to ionization which
is realized numerically by absorbing boundary conditions. From the time evolution of the 
level depletions and ionizations per state, $N_\alpha(t)$ and $N_{{\rm esc},\alpha}(t)$ respectively, 
defined in Eq.~\eqref{eq:depletion},
%These reduce in the course of time the norm of s.p. wave function
%leaving a particle content $N_{\alpha}$ in s.p. state $\alpha$ and its complement
%\begin{equation} 
%  N_{\alpha}(t) 
%  = 
%  \int d\mathbf{r}  |\varphi_{\alpha}(\mathbf{r},t)|^{2}
%  \quad,\quad
%  \tilde{N}_{\alpha}
%  =
%  1-N_{\alpha}
%  \quad.
%\end{equation}
%Once having the $N_{\alpha}(t)$, the further steps proceed as
%previously.
one starts with the following relation
\begin{equation}
\label{eq:combi}
  1 
  = \sum_{k=0}^{\Omega} P^{(+k)}(t)
  = 
  \prod_{\alpha=1}^{\Omega}\Big[ N_\alpha(t) + N_{{\rm esc},\alpha}(t))\Big]
  \quad.
\end{equation}
The product in the right-hand side is then expanded and the resulting
terms are rearranged, collecting terms containing $k$ factors
$N_{\alpha}^k\, \left(N_{{\rm esc},\alpha}\right)^{\Omega-k}$ which
allows one to identify them to the ionization probability $P^{(+k)}$
\cite{Ull97a}. For examples of application and details see, e.g.,
\cite{Rei98a, Cal00}.

The irradiation of the C$_3$ chain by an intense laser pulse is taken as an example
and various time evolutions are shown in figure~\ref{fig:probab}.
\begin{figure}[htbp]
\centerline{
%\includegraphics[width=0.3\columnwidth]{figures/H2O-sp}
\includegraphics[width=\columnwidth]{figures/C3-probab-3}
}
\caption{The C$_3$ chain molecule excited by a laser pulse with
  $\omega_\mathrm{las}=5.4$ eV, $I=10^{14}$ W/cm$^2$,
  $T_\mathrm{pulse}=10$ fs, and with polarization along the $z$ direction ($=$ molecule axis), 
  leading to an average ionization of 0.8
  electrons. As functions of time: (bottom left) $D_z$, dipole moment in $z$ direction, (top left)
  total ionization $N_\mathrm{esc}$ and ionizations per state $N_{{\rm esc},\alpha}$ 
 and (right) charge-state probabilities $P^{(+k)}$ in logarithmic scale.}
\label{fig:probab}
\end{figure}
The dipole moment (lower left panel) gives an impression of the
dynamical response of the molecule.  It shows the typical pattern for
an off-resonant excitation.  The dipole signal follows the laser
profile and some small asymptotic oscillations remain, due to a small
excitation of the system's eigenmodes. The total ionization (top red
curve in upper
left panel) increases in the region of large dipole amplitudes and then
levels off. We also show the ionizations from the HOMO, HOMO$-1$ and
two degenerate HOMO$-{2,3}$ levels.  These are the key ingredients in
evaluating the charge-state probabilities $P^{(+k)}$.  The emerging
$P^{(+k)}$ are shown in the right panel.  Their time evolution
corresponds to the pattern of total ionization.  The neutral state loses
probability with increasing ionization while all other take

increasingly their share. Finally, the average charge state of $Q=0.8$,
read off from the total ionization,
is mainly distributed with 40\% 
of charge state $Q=0$ and 40\% of charge state $Q=1$. Charge state $+2$
gather appreciable 14\% and the higher charges share the leftovers
with rapidly decreasing amounts (only visible on logarithmic
scale). The distribution visualizes a subtle feature of TDLDA in
connection with absorbing boundary conditions: it generates inevitably
a mixed state (which becomes apparent in a natural-orbital
representation \cite{Vin17a}) and this state embraces very different
charge states when decomposed properly.  The different charges produce
different Coulomb fields. TDLDA can describe only the average charge
and an average mean field.

So far, we have discussed the case of pure mean-field states with
integer occupation. RTA inevitably produces mixed states with
fractional occupations $w_\alpha$.  The extension to this situation
is formally trivial and actually implemented in QDD, provided that we
merely generalize the level depletion of state $\alpha$ as
%
$N_\alpha(t)=w_\alpha\langle \varphi_{\alpha}(t)|\varphi_\alpha(t)\rangle$
%
and apply the combinatorial decomposition as before.  Although
technically simple, the physical interpretation of the resulting
$P^{(+k)}$ is involved.  Fractional occupations $w_\alpha$ as generated
by RTA produces also a spread in charge states which has nothing to do
with ionization, but reflects a general weakness of a thermal
mean-field state, namely that it represents unavoidably an ensemble
covering different electron numbers \cite{Rei98aB}.  The resulting
$P^{(+k)}$ of RTA with absorbing boundary conditions thus mixes effects
from ionization with effects from thermalization. It is a task for
future research to disentangle these two contributions.
...
normalization.  The angular segments in Fig.~\ref{fig:mask} schematically
...
A Photo-Electron Spectrum (PES) can be deduced from the temporal phase
oscillations of the s.p.  wave functions
$\varphi_\alpha(\mathbf{r}_\mathcal{M},t)$ at measuring points
$\mathbf{r}_\mathcal{M}$ close to the absorbing bounds, see
Fig.~\ref{fig:mask} \cite{Poh00}.  The result for the PES sampled at
...
$F(t)=\int_0^t  \textrm dt'\,f(t')\exp{(-\mathrm{i}\omega_\mathrm{las}t')}$ is the
time integrated laser pulse envelop introduced in
Eq.~(\ref{eq:laserp}).  
...
$\mathbf{k}=\mathbf{e}_{\mathbf{k}}\sqrt{2 m_e E_\mathrm{kin}}$. 
The energy is read off from the phase oscillations of
$\varphi(\mathbf{r}_\mathcal{M},t)$ 
and the direction
$\mathbf{e}_{\mathbf{k}}\Rightarrow\Omega_\mathcal{M}$ from the
position of the measuring point.
The practical evaluation of PES is done in a two-step
process. QDD prints out the phase information at the measuring
point to a file which is afterward analyzed by an auxiliary
program, for details, see the Supplemental material, 
section \ref{man:sec:PES} therein.
...
order of magnitude, an intensity for long laser pulses impinging on Na
clusters is $I\approx{10}^{15}$W/cm$^2$. For details, see
\cite{Din13a}. 

This evaluation of PES through the above explained phase sampling
is extremely efficient. However, it is not yet applicable to
dissipative dynamics as RTA, because that causes jumps in the
wave functions and thus spoils phase information. A proper extension of
...
\centerline{
%\includegraphics[width=0.3\columnwidth]{figures/H2O-sp}
%\includegraphics[width=0.8\columnwidth]{figures/PES-offres-lower_intensity-4}
\includegraphics[width=0.8\columnwidth]{figures/PES_h2o}
}
...
figure) thus remain almost constant in time. The PES clearly exhibits
...
PES is thus a standard tool to access the single particle energy spectrum of a
...
system, in particular on the time evolution of the s.p. energy spectrum.  
...
It serves as a direct indicator of thermalization and allows one to 
...
Note that $S$ remains strictly zero at TDLDA   level with 
constant values of $w_\alpha$ = 0 or 1.
...
\end{figure}
A rough schematic overview is given in Fig.~\ref{fig:tree_main}. It
...
A simple way to visualize  RTA dynamics is to consider an
...
We  take here such an initial boost along the $z$ direction applied on
...
delivering three different initial  excitation energies $E^*$.
\begin{figure}[htbp]
\begin{center}
 \includegraphics[width=0.6\columnwidth]{figures/na11p_lda_rta}
\caption{Time evolution of the one-body entropy (bottom), the total
  ionization (middle) and the envelop of the dipole moment in $z$ direction 
...
that we chose to plot the envelop of the dipole amplitude rather than
...
TDLDA \cite{Cal00}. Dissipation becomes effective in the second phase
...
striking difference occurs in terms of the entropy $S$. It vanishes
...
$\omega_\mathrm{las}=11.4$~eV, and one off-resonance with
$\omega_\mathrm{las}=10.2$ eV, the other laser characteristics
...
$\omega_\mathrm{las}=11.4$~eV for the off-resonance case (left column)
and $\omega_\mathrm{las}=10.2$ eV for the on-resonance case (right
...
a laser frequency slightly above the lowest excitation energy mode
(10.2 eV).  The early stages of irradiation progressively ionize the
molecule which produces a slight blue-shift of the resonance and thus
places the system, still during the pulse, exactly on top of the
...
whose envelope deviates soon from the pulse envelope and which
continues to oscillate long after the pulse is over leading also to
ongoing electron emission. The mechanism is well known and was used to
analyze expansion of clusters irradiation by strong lasers
\cite{Hoh94,Koe99}.  Even if the mechanism is well understood, it has
mostly been described theoretically using TDLDA-like approaches, see
e.g. \cite{Sur00}.  The major interest of the case here is the
striking difference between RTA and TDLDA dynamics during the second
phase of the response.  While TDLDA dynamics leaves long lasting
dipole oscillations, dissipation in RTA manages to curb down the
dipole signal by turning energy away from the dipole channel and
distributing it over the great manifold of other excitation modes. A
remarkable feature is seen in the energy absorption from the laser
field (right middle panel).  TDLDA shows strong oscillations between
absorption and (induced) emission of energy very similar to Rabi
oscillations \cite{Vin18b}.  RTA reduces these oscillations because
attenuation of the dipole reduces emission. This, in turn, gives way
to absorb more energy from the laser field. 
...
case.  Only small differences can  be spotted on ionization, with RTA
...
dependence. (less symmetric systems usually require some
...
\end{center}
\end{figure}
Figure~\ref{fig:na11p_pad} shows the PAD  in RTA (solid
lines) and TDLDA (dashed curves), obtained after initial boost
excitation in Na$_{11}^+$. 
 Results from three different boost
momenta, and consequently three different initial excitation energies $E^*$
as indicated, are compared.  The stronger the excitation, the more
forward peaked are the PAD.  This is a well known and plausible
...
quantum Relaxation-Time Approximation which nevertheless includes essential
...
structural details can be energetically overlooked. The set of parameters
...
This first release of QDD contains the full capabilities of
  ...
  latter, we work on an implementation of Stochastic Time-Dependent
...
were to be changed slightly. The Fermi distribution is obtained
...
\begin{equation}
  \tilde{N}
  =
  \sum_\alpha \tilde w_\alpha
  \quad,\quad
  \tilde E_\mathrm{sp}
  =
  \sum_\alpha \tilde w_\alpha \varepsilon_\alpha
  \quad.
\label{eq:statusN}
\end{equation}
...
\begin{equation}
  \sum_\alpha\big(s(\tilde w_\alpha+\delta w_\alpha)-s(\tilde w_\alpha)\big)^2
  \stackrel{!}{=}
  \mbox{min.}
  \quad,
\end{equation}
\end{subequations}
together, again, with the constraints (\ref{eq:constrN}).
This amounts to the variational condition
\begin{equation}
  \frac{\textrm d\qquad }{\textrm d \,\delta w_\alpha}
  \left[
     \sum_\alpha\big(s(\tilde w_\alpha+\delta w_\alpha)-s(\tilde w_\alpha)\big)^2
     -
     \lambda\sum_\alpha (\tilde w_\alpha+\delta w_\alpha)
     -
     \mu\sum_\alpha (\tilde w_\alpha+\delta w_\alpha)\varepsilon_\alpha
  \right]
  =
  0
  \quad.
...
\end{eqnarray}
\end{subequations}
where $\delta N, \delta E_\mathrm{sp}$, are the differences between
  status and target values of $N, E_\mathrm{sp}$, as outlined in Eqs.~\ref{eq:constrN} 
  and \ref{eq:statusN}, and $s'$ denotes the derivative of $s$ with respect to $w$ ($s'=\textrm ds/\textrm dw$). 
Note that this rather robust scheme is applied for the spin-up and
spin-down subsets separately.


\section{Dimensionless quadrupole moments}
\label{sec:dimlessquad}

In section \ref{sec:shapes}, we introduced the basic form parameters
of a density distribution. They are plagued by the inconvenience to
scale with system size. To concentrate on shape, the concept of
dimensionless quadrupole moments was used since long in nuclear
physics \cite{Hil53,Boh69}. It turned out also be useful in cluster
physics \cite{Hee93,Bra93} and surely helps to sort the gross
structure of molecules. We review here briefly the steps from Cartesian
quadrupole moments to dimensionless ones.

The first step is to map  Cartesian quadrupole moments into spherical 
quadrupole moments $Q_{2\mu}$ \cite{Edm57}
\begin{equation}
  Q_{20}
  =
  \sqrt{\frac{5}{2\pi}}\,\frac{Q_{zz}}{3}
  \;,\;
  Q_{2\,\pm 1}
  =
  \mp\sqrt{\frac{15}{8\pi}}\,(Q_{zx}\pm\mathrm{i}Q_{zy)}
  \;,\;
  Q_{2\,\pm 2}
  =
  \sqrt{\frac{15}{32\pi}}\,(Q_{xx}-Q_{yy}\pm\mathrm{i}Q_{xy)}
  \;.
\end{equation}
The dimensionless moments are then obtained as
\begin{subequations}
\begin{equation}
  \alpha_{lm}(t)
  =
  \frac{4\pi}{5}
  \frac{Q_{2\mu}}{N_\mathrm{el}\,r_\mathrm{rms}^2}
  \quad,
\label{eq:def_dimless}
\end{equation}
\end{subequations}
A simple signature of shape can already be read off here: Axially
symmetric systems aligned along $z$-axis are distinguished by
$\alpha_{lm\!\neq\!0}=0$.

However, the example of axial systems indicates a next problem, namely
the orientation of the system.  The appearance of
$\alpha_{lm\!\neq\!0}\neq 0$ can have two causes: first, the system is
not aligned along $z$-axis, and second, there is a true breaking of
axial symmetry. The first action is then to determine the principle
axes of the system and to rotate the system such that the $z$-axis is
identical with the dominant principal axis.  This leads by definition
to $\alpha_{2\,\pm 1}=0$ and $\alpha_{2\,+2}=\alpha_{2\,-2}$. Axial
symmetry is truly broken if we then still find
$\alpha_{22\!\neq\!0}\neq 0$, signaling triaxial shapes. One
often regroups the quadrupole deformation parameters into total
deformation $\beta_2$ and triaxiality $\gamma$ as
\begin{equation}
  \beta_2
  =
...
The same definitions apply to ionic shapes if we replace $\int \textrm
d \mathbf r\,\varrho(\mathbf{r})\,r_ir_j$ by $\sum_I R_{I,i}R_{I,j}$
in Eqs. (\ref{eq:moments}).

\section{Handling of basis sets in RTA}
\label{sec:basisRTA}


\subsection{Transformation to natural orbitals}
\label{sec:natorb}

Before entering the RTA step, we have a one-body density matrix
obtained from a couple of TDLDA steps
\begin{equation}
  \hat{\rho}
  \equiv
  \hat{\rho}_\mathrm{mf}
  =
  \sum_\alpha|\varphi_\alpha^\mathrm{(mf)}\rangle 
  \ w_\alpha^\mathrm{(mf)} \ \langle\varphi_\alpha^\mathrm{(mf)}|
  \;.
\end{equation}
The absorbing boundary conditions remove electron content from
the wavefunctions which thus lose their strict ortho-normality
leading to a non-unit norm matrix 
\begin{equation}
  \mathcal{N}_{\alpha\beta}
  =
  \langle\varphi_\alpha^\mathrm{(mf)}|\varphi_\beta^\mathrm{(mf)}\rangle 
  \;.
\end{equation}
We first need to represent the actual one-body density matrix
in terms of natural orbitals 
\begin{equation}
  \hat{\rho}
  =
  \sum_i|\varphi_i^\mathrm{(nat)}\rangle w_i^\mathrm{(nat)}
  \langle\varphi_i^\mathrm{(nat)}|
\end{equation}
which are ortho-normal, i.e.
$\langle\varphi_i^\mathrm{(nat)}|\varphi_j^\mathrm{(nat)}\rangle=\delta_{ij}$.
The loss of electron content is taken up by a change of the
occupations to $w_i^\mathrm{(nat)}$.

The natural orbitals are defined by the eigenvalue equation
\begin{equation}
  \hat{\rho}|\varphi_i^\mathrm{(nat)}\rangle
  =
  |\varphi_i^\mathrm{(nat)}\rangle w_i^\mathrm{(nat)}
  \;.
\label{eq:eigenNat}
\end{equation}
We expand them in terms of the TDLDA obitals 
\begin{equation}
  |\varphi_i^\mathrm{(nat)}\rangle
  =
  \sum_\alpha|\varphi_\alpha^\mathrm{(mf)}\rangle W_{\alpha i}
  \;.
\label{eq:expandW}
\end{equation}
The task is now to determine the transformation matrix $W_{\alpha i}$.

We insert the expansion (\ref{eq:expandW}) into the eigenvalue
equation (\ref{eq:eigenNat}) and obtain a matrix equation
\begin{eqnarray}
  \rho_{\alpha\beta}W_{\beta i}
  &=&
  \mathcal{N}_{\alpha\beta}W_{\beta i}w_i^\mathrm{(nat)}
  \;,
\label{eq:eigenN}\\
  \rho_{\alpha\beta}
  &=&
  \langle\varphi_\alpha^\mathrm{(mf)}|\hat{\rho}
  |\varphi_\beta^\mathrm{(mf)}\rangle   
  =
  \sum_\gamma\mathcal{N}_{\alpha\gamma}w_\gamma^\mathrm{(mf)}\mathcal{N}_{\gamma\beta}
  \;.
\nonumber
\end{eqnarray}
We first determine eigenvalues and eigenvectors of $\hat{\mathcal{N}}$
through
\begin{equation}
  \mathcal{N}_{\alpha\beta}V_{\beta l}
  =
  V_{\alpha l}n_l
\end{equation}
and use that to build the inverse root of  $\hat{\mathcal{N}}$ as
\begin{equation}
  (\mathcal{N}^{-1/2})_{\alpha l}
  =
  V_{\alpha l}n^{-1/2}_l
  \;.
\end{equation}
With that, we can map the eigenvalue equation (\ref{eq:eigenN}) into
standard form
\begin{equation}
  (\mathcal{N}^{-1/2})^\dagger_{l\alpha}\rho_{\alpha\beta}(\mathcal{N}^{-1/2})_{\beta l'}
  U_{l'i}
  =
  U_{li}w_i^\mathrm{(nat)}
  \;.
\label{eq:eigenfin}
\end{equation}
Having determined the eigenvalues $w_i^\mathrm{(nat)}$ and
eigenvectors $U_{li}$, we realize that
\begin{equation}
  \rho_{\alpha\beta}(\mathcal{N}^{-1/2})_{\beta l'}U_{l'i}
  =
  (\mathcal{N}^{+1/2})_{\alpha l}U_{li}w_i^\mathrm{(nat)}
  =
  \mathcal{N}_{\alpha\beta}\;(\mathcal{N}^{-1/2})_{\beta l}U_{li}w_i^\mathrm{(nat)}
  \;.
\end{equation}
and conclude that the searched transformation for the expansion
(\ref{eq:eigenN}) is finally composed as
\begin{equation}
  W_{\alpha i}
  =
  (\mathcal{N}^{-1/2})_{\alpha l}U_{li}
  \:.
\end{equation}
The new occupation numbers $w_i^\mathrm{(nat)}$ were already given
from solving the eigenvalue equation (\ref{eq:eigenfin})



\subsection{Mixing of two one-body matrices}
\label{sec:mix}

At the end of one RTA step, we mix two one-body density matrices as
\begin{equation}
  \hat{\rho}_\mathrm{(mix)}
  = 
  (1-\eta)\hat{\rho}_\mathrm{(mf)} +\eta\hat{\rho}_\mathrm{(eq)}
  \;,
\end{equation}
see step \ref{it:compo} in section \ref{sec:summaryRTA}. The aim is to
produce eventually $\hat{\rho}_\mathrm{(mix)}$ in the basis of its
natural orbitals. The problem is that the constituents
\begin{eqnarray*}
  \hat{\rho}_\mathrm{(mf)}
  &=&
  \sum_{\alpha=1}^\Omega|\varphi_\alpha^\mathrm{(mf)}\rangle 
  \ w_\alpha^\mathrm{(mf)} \ \langle\varphi_\alpha^\mathrm{(mf)}|
  \;,
\\
  \hat{\rho}_\mathrm{(eq)}
  &=&
  \sum_{\alpha=1}^\Omega|\varphi_\alpha^\mathrm{(eq)}\rangle 
  \ w_\alpha^\mathrm{(eq)} \ \langle\varphi_\alpha^\mathrm{(eq)}|
\end{eqnarray*}
are represented on different sets of s.p. states from which
$\{\varphi_\alpha^\mathrm{(mf)},\alpha=1...\Omega\}$ is not even
ortho-normalized.

We combine the two sets of s.p. states to one super-set
$\{\phi_A,A=1...2\Omega\}$ as
\begin{subequations}
\begin{eqnarray}
  \phi_A
  &=&
  \left\{
  \begin{array}{lcl}
    \varphi_\alpha^\mathrm{(mf)} &\mbox{for}& A=\alpha\in\{1...\Omega\}
  \\
    \varphi_{\alpha'}^\mathrm{(eq)} &\mbox{for}& A=\alpha'+\Omega
     \;,\;\alpha'\in\{1...\Omega\}
  \end{array}
  \right.
\\[8pt]
  ...
  \left\{
  \begin{array}{lcl}
    w_\alpha^\mathrm{(mf)} &\mbox{for}& A=\alpha\in\{1...\Omega\}
  \\
    w_{\alpha'}^\mathrm{(eq)} &\mbox{for}& A=\alpha'+\Omega
     \;,\;\alpha'\in\{1...\Omega\}
  \end{array}
  \right.
\end{eqnarray}
\end{subequations}
This yields a one-body density in double-space
\begin{equation}
  \hat{R}
  =
  \sum_{A=1}^{2\Omega}|\phi_A\rangle \mathcal{W}_A\langle\phi_A|
\end{equation}
with a basis $\{\phi_A\}$ which is not northo-normalized.  That is
exactly the task to find natural orbitals which was solved in section
\ref{sec:natorb}. We undertake the same steps (of course, in doubled
space) and end up with
\begin{equation}
  \hat{R}
  =
  \sum_{A=1}^{2\Omega}|\phi_A^\mathrm{(nat)}\rangle {W}_A^\mathrm{(nat)}
   \langle\phi_A^\mathrm{(nat)}|
\end{equation}
whereby we take care to order the states to a sequence of decreasing
occupations ${W}_A^\mathrm{(nat)}$. This one-body density is still
given in a space which is twice as large as the s.p. space before
mixing. But the occupations for large $A$ are extremely small. This
allows to reduce $\hat{R}$ back to standard size and in natural
orbitals as
\begin{equation}
  \hat{\rho}_\mathrm{(mix)}
  = 
  \sum_{\alpha=1}^\Omega|\varphi_\alpha^\mathrm{(mix)}\rangle 
  \ w_\alpha^\mathrm{(mix)} \ \langle\varphi_\alpha^\mathrm{(mix)}|
  \;,\;
  \varphi_\alpha^\mathrm{(mix)}
  =
  \phi_\alpha^\mathrm{(nat)}
  \;,\;
  w_\alpha^\mathrm{(mix)}
  =
  \mathcal{W}_\alpha^\mathrm{(nat)}
  \;.
\end{equation}
